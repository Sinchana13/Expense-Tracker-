...
/*
	...
	function encryptIt($value) {
		...
		$encoded = base64_encode($iv . openssl_encrypt($value, 'aes-256-cbc', md5($encodeKey), 0, $iv));
    ...
    $ivLength = openssl_cipher_iv_length('aes-256-cbc');
    $iv = substr(base64_decode($value), 0, $ivLength);
    $encryptedText = substr(base64_decode($value), $ivLength);

    $decoded = openssl_decrypt($encryptedText, 'aes-256-cbc', md5($decodeKey), 0, $iv);
